        ...
        await page.evaluate(async ([filename, type, host, localhost]) => {
            ...
            class ContentSplitter {
                constructor(contentRoot, width, height) {
                    ...
                    this.splitContentRoot = contentRoot.cloneNode(false)
                    this.currentDiv = this.contentRoot.cloneNode(false)
                }
                ...
                _splitContent(content) {
                    ...
                    console.log("currentHeight " + this.currentHeight + " contentHeight " + content.clientHeight * this.scale)
                    if (this.fitsInPage(content)) { // Content can be added to current page
                        if (content !== this.contentRoot) {
                            this.currentDiv.appendChild(content.cloneNode(true))
                            this.currentHeight += content.clientHeight * this.scale
                        } else { // If content from root fits in one page
                            this.splitContentRoot = content.cloneNode(true)
                        }
                    } else { // Content has to be split
                        this.currentDiv = content.cloneNode(false)
                        const childrenDiv = Array.prototype.slice.call(content.children).filter(child => child.nodeType === 1)
                        if (childrenDiv.length > 0) {
                            for (let i = 0; i < childrenDiv.length; i++) {
                                this._splitContent(childrenDiv[i])
                            }
                        } else { // Content cannot be split
                            this.currentDiv = content.cloneNode(false)
                            let currentReconstructedNode = content
                            while (currentReconstructedNode !== this.contentRoot) {
                                const parent = currentReconstructedNode.parentNode
                                const reconstructedNode = parent.cloneNode(false)
                                reconstructedNode.appendChild(currentReconstructedNode)
                                currentReconstructedNode = parent
                            }
                            this.splitContentRoot.appendChild(currentReconstructedNode)
                            this.currentHeight = 0
                            if (!this.fitsInPage(content)) { // Unsplittable content is too big for one page
                ...
                iterateChildrenDiv(current) {
                    const children = current.children.length > 0 ? Array.prototype.slice.call(current.children) : []
                    return current.nodeType + " " + current.className + "\n\t" + (children.length > 0 ? children.filter(child => child.nodeType === 1).map(child => this.iterateChildrenDiv(child)) : "")
                }

                get childrenDivString() {
                    return this.iterateChildrenDiv(this.contentRoot)
                }
            ...
            switch (type) {
                case "mode":
                    await generateFormationBooklet(filename)
                    return
                case "module":
                    //await modulePDF(filename)
                    const doc = new jsPDF(options)
                    const keks = new ContentSplitter(document.querySelector(".pdf-content"), doc.internal.pageSize.getWidth() - (marginLeft + marginRight), doc.internal.pageSize.getHeight() - (marginTop + marginBot))
                    keks.splitContent()
                    console.log(keks.contentRoot)
...
function test() {
    puppeteer.launch({ headless: false }).then(browser => {
        ...
        pdfGenerator.generatePDFFromURL(localhost + "bachelor/ingenierie-et-architecture/tic/isc/iscs/pt/sld/", "pdf", "test.pdf", "module").then(_ => {
            browser.close().then()
        })
    })
}
